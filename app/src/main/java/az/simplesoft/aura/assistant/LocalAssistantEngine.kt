...
class LocalAssistantEngine(
    ...
    private fun registryAction(match: RegistryIntentMatch, language: AssistantLanguage): AssistantReply? = when (match.intentId) {
        ...
        "stop_after_track" -> AssistantReply(MusicIntent.StopAfterTrack, localized(language, "Остановлюсь после этой песни.", "Bu mahnıdan sonra dayanacağam.", "I'll stop after this song."), language)
        "cancel_sleep_timer" -> AssistantReply(MusicIntent.CancelSleepTimer, localized(language, "Таймер сна выключен.", "Yuxu taymeri söndürüldü.", "Sleep timer cancelled."), language)
        ...
        "equalizer_off" -> AssistantReply(MusicIntent.DisableEqualizer, localized(language, "Эквалайзер выключен.", "Ekvalayzer söndürüldü.", "Equalizer off."), language)
        "equalizer_bass" -> AssistantReply(MusicIntent.SetEqualizer(EqualizerPreset.BASS), localized(language, "Включаю басовый пресет.", "Bas preseti aktivdir.", "Bass preset on."), language)
        "equalizer_vocal" -> AssistantReply(MusicIntent.SetEqualizer(EqualizerPreset.VOCAL), localized(language, "Включаю пресет вокала.", "Vokal preseti aktivdir.", "Vocal preset on."), language)
        "equalizer_rock" -> AssistantReply(MusicIntent.SetEqualizer(EqualizerPreset.ROCK), localized(language, "Включаю рок-пресет.", "Rok preseti aktivdir.", "Rock preset on."), language)
        "equalizer_acoustic" -> AssistantReply(MusicIntent.SetEqualizer(EqualizerPreset.ACOUSTIC), localized(language, "Включаю акустический пресет.", "Akustik preseti aktivdir.", "Acoustic preset on."), language)
        "equalizer_cycle" -> AssistantReply(MusicIntent.CycleEqualizer, localized(language, "Переключаю пресет эквалайзера.", "Ekvalayzer preseti dəyişir.", "Changing equalizer preset."), language)
        ...
        "more_like_this" -> AssistantReply(MusicIntent.MoreLikeThis, localized(language, "Подбираю ещё похожее.", "Buna oxşar daha çox musiqi seçirəm.", "Finding more like this."), language)
        "not_this" -> AssistantReply(MusicIntent.NotThis, localized(language, "Убираю этот вариант.", "Bu variantı silirəm.", "Skipping this option."), language)
        "clear_memory" -> AssistantReply(MusicIntent.ClearMemory, localized(language, "Удаляю сохранённую память о тебе.", "Sənin haqqındakı yaddaşı silirəm.", "Clearing the memory saved about you."), language)
